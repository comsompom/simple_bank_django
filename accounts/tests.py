...
class AccountServiceTests(TestCase):
    ...
    def test_create_account_for_user_without_bonus_starts_at_zero(self):
        ...
        account = create_account_for_user(user=user, swift_code="BANKDEFF", with_welcome_bonus=False)
...
class AccountApiTests(APITestCase):
    ...
    def test_account_me_returns_account_details(self):
        response = self.client.get("/api/v1/accounts/me/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["account_number"], self.user.bank_account.account_number)
